"""
Extension — LDA Hyperparameter Tuning for Keyboards & MIDI
===========================================================
Mirrors lda_tune_params_v2.py exactly — same 8-config grid, same evaluation.
Reuses keyboards_tokens.pkl/keyboards_dict.pkl/keyboards_corpus.pkl from preprocess_keyboards.py.

Output: tune_results_keyboards.txt
  → Read this file to find best (num_topics, alpha, eta, passes)
  → Copy those values into assign_topics_keyboards.py
"""
...
RESULTS_OUT = os.path.join(SCRIPT_DIR, 'tune_results_keyboards.txt')
...
for path in [TOKENS_FILE, DICT_FILE, CORPUS_FILE]:
    if not os.path.exists(path):
        raise FileNotFoundError(
            ...
            "Run preprocess_keyboards.py first."
        )
...
# ── Save results ──────────────────────────────────────────────────────────────
lines = ["LDA Hyperparameter Tuning — Keyboards & MIDI\n", "=" * 70 + "\n\n"]
lines.append("Copy the BEST row values into assign_topics_keyboards.py:\n")
...
print("\nNEXT STEP:")
print("  1. scp tune_results_keyboards.txt to your laptop")
print("  2. Read it — find best params, read topic words for best model")
print("  3. Fill in NUM_TOPICS/ALPHA/ETA/PASSES + FINAL_TOPICS in assign_topics_keyboards.py")
print("  4. sbatch run_assign_keyboards.sh")
print("Done.")
