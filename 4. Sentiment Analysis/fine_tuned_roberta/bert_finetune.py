...
# ── Argument parser (lets you override settings from command line) ─────────────
parser = argparse.ArgumentParser(description="Fine-tune RoBERTa for sentiment analysis")
parser.add_argument("--data",       type=str,   default=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'guitars_with_topics_v2.parquet'))
parser.add_argument("--model",      type=str,   default="roberta-base")
parser.add_argument("--output_dir", type=str,   default="./roberta_sentiment_model")
...
# Rating 4-5 → positive (1) | Rating 1-2 → negative (0) | Rating 3 → excluded
def assign_label(rating):
    if rating >= 4:
        ...
        return None  # neutral — excluded from training
...
# Keep full dataset for final per-topic inference (includes rating=3)
df_all = df.copy()
...
# Split training set further to create a validation set (10% of training)
...
class ReviewDataset(Dataset):
    """Wraps a DataFrame of reviews + labels into a PyTorch Dataset."""
...
# Macro F1 treats both classes equally — important for imbalanced data (85% positive)
...
# ==============================================================================
# STEP 6 — Evaluate on Test Set (manual inference, no Trainer callbacks)
# ==============================================================================
...
# ==============================================================================
# STEP 7 — Per-Topic Sentiment Inference (Main Output)
# ==============================================================================
...
infer_dataset = ReviewDataset(
    inference_df["text"],
    [-1] * len(inference_df),   # placeholder labels
    tokenizer,
    args.max_length
)
